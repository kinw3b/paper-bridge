...
**This is not a standalone tool.** It is the Capture Tool used in step 1.3 of the web2html skill — the human pass that records dropdown, button hover, and component states from a live page into [Paper](https://paper.design). Session order is Dropdowns → Buttons → Components → Done. Tags come from 1.2 `layer-ids.json`; 2.2.a applies that map. Use it with the skill, not on its own.
