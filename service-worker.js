...
const CONTENT_CSS = ["content/capture.css"];
...
async function pageReady(tabId) {
  try {
    const [result] = await chrome.scripting.executeScript({
      target: { tabId },
      func: () => Boolean(
        globalThis.PaperCaptureNaming?.componentName
...
async function inject(tabId) {
  for (let attempt = 0; attempt < 5; attempt += 1) {
    if (await pageReady(tabId)) return { ok: true, reused: true };
    try {
      ...
      await chrome.scripting.executeScript({
        target: { tabId },
        files: CONTENT_HELPERS,
      });
      await chrome.scripting.executeScript({
        target: { tabId },
        func: () => { delete window.__PAPER_CAPTURE_EXTENSION__; },
      });
      await chrome.scripting.executeScript({
        target: { tabId },
        files: ["content/capture.js"],
      });
      if (await pageReady(tabId)) return { ok: true, reused: false };
    } catch (error) {
